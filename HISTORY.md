...
### 2025년 7월 (상반부 외 추가 요약)

- **UI/UX 및 내비게이션 개선**:
  - CSS 기반 스크롤 리빌 애니메이션과 뷰 전환 개선으로 부드러운 페이지 전환 구현
  - 네비게이션/댓글 UI 구조 개선 및 한국어 로케일·메시지 정비
- **검색/임베딩·RSS·도메인**:
  - 기사 임베딩 모델 및 검색 스코프 고도화
  - RSS 피드 추가 및 캐시/라우팅 정책 정리
  - 서비스 도메인을 `ruby-news.kr`로 전환하고 리다이렉트 처리 도입
- **품질/로깅/에이전트**:
  - Article 라이프사이클 및 로깅 구조 정리
  - 에이전트 관련 문서/구성 일원화 및 코드 리뷰 워크플로우 개선
